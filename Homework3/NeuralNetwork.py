...
class NeuralNetwork:
    def __init__(self,size_list):
        ...
        #layer_matrix = torch.tensor([[0.35, 0.15, 0.25],[0.35, 0.20, 0.30]])
        #layer_matrix = layer_matrix.type(torch.DoubleTensor)
        #self.__Theta["theta"+str(1)] = layer_matrix
        #layer_matrix = torch.tensor([[0.6, 0.4, 0.5],[0.6,0.45,0.55]])
        #layer_matrix = layer_matrix.type(torch.DoubleTensor)
        #self.__Theta["theta"+str(2)] = layer_matrix
    ...
    def updateParams(self,eta):
        for i in range(len(self.__size_list)-1,0,-1):
            self.__Theta["theta"+str(i)] = self.__Theta["theta"+str(i)]-eta*self.__dE_dTheta["layer"+str(i)]
        return self.__Theta